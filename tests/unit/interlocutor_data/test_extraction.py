"""Unit tests for InterlocutorDataSchemaExtractor.
...
from ruche.interlocutor_data.extraction import (
    ExtractionResult,
    FieldDefinitionSuggestion,
    InterlocutorDataSchemaExtractor,
)
...
def extractor_no_llm():
    ...
    return InterlocutorDataSchemaExtractor(llm_executor=None)


@pytest.fixture
def extractor_with_llm(mock_llm):
    """Create extractor with mock LLM."""
    return InterlocutorDataSchemaExtractor(llm_executor=mock_llm)
...
class TestCreateFieldDefinitions:
    ...
    def test_create_definitions(self, extractor_no_llm, tenant_id, agent_id):
        """Test creating InterlocutorDataField objects."""
        suggestions = [
            FieldDefinitionSuggestion(
                name="email",
