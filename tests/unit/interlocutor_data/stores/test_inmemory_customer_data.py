"""Tests for InMemoryInterlocutorDataStore."""
...
@pytest.fixture
def store() -> InMemoryInterlocutorDataStore:
...
@pytest.fixture
def sample_profile(tenant_id) -> InterlocutorDataStore:
...
class TestProfileOperations:
    ...
    @pytest.mark.asyncio
    async def test_get_by_interlocutor_id(self, store, sample_profile, tenant_id):
        """Should retrieve profile by customer ID."""
        await store.save(sample_profile)
        retrieved = await store.get_by_interlocutor_id(
            tenant_id, sample_profile.interlocutor_id
        )

        assert retrieved is not None
        assert retrieved.interlocutor_id == sample_profile.interlocutor_id
