"""Unit tests for InterlocutorDataStoreCacheLayer.
...
from ruche.config.models.storage import RedisProfileCacheConfig
...
@pytest.fixture
def sample_profile(tenant_id, profile_id, interlocutor_id):
...
def cached_store(backend_store, mock_redis, config):
    ...
    return InterlocutorDataStoreCacheLayer(backend_store, mock_redis, config)
...
class TestCacheHitBehavior:
    ...
    @pytest.mark.asyncio
    async def test_get_by_interlocutor_id_cache_hit(
        self, cached_store, mock_redis, sample_profile, tenant_id, interlocutor_id
    ):
        """Test cache hit for get_by_interlocutor_id."""
        mock_redis.get.return_value = sample_profile.model_dump_json()

        result = await cached_store.get_by_interlocutor_id(tenant_id, interlocutor_id)
...
class TestCacheMissBehavior:
    ...
    @pytest.mark.asyncio
    async def test_get_by_interlocutor_id_cache_miss(
        self, cached_store, backend_store, mock_redis, sample_profile, tenant_id, interlocutor_id
    ):
        """Test cache miss for get_by_interlocutor_id."""
        mock_redis.get.return_value = None
        await backend_store.save(sample_profile)

        result = await cached_store.get_by_interlocutor_id(
            tenant_id, sample_profile.interlocutor_id
        )

        assert result is not None
        # Should cache both by interlocutor_id and profile_id
...
class TestCacheInvalidation:
    ...
    async def test_add_asset_invalidates_cache(
        ...
        from ruche.interlocutor_data.models import ProfileAsset
...
class TestRedisFailureFallback:
    ...
    async def test_fallback_disabled_raises_error(
        ...
        cached_store = InterlocutorDataStoreCacheLayer(backend_store, mock_redis, config)
        mock_redis.get.side_effect = redis.RedisError("Connection failed")
...
class TestFieldDefinitionCaching:
    ...
    async def test_get_field_definitions_caches(
        ...
        from ruche.interlocutor_data.enums import ValidationMode
...
class TestScenarioRequirementCaching:
    ...
    async def test_get_scenario_requirements_caches(
        ...
        from ruche.interlocutor_data.models import ScenarioFieldRequirement
        from ruche.interlocutor_data.enums import RequiredLevel, FallbackAction
