...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/models/ -v
uv run pytest tests/unit/pipelines/focal/loaders/ -v
uv run pytest --cov=ruche/pipelines/focal --cov-report=term-missing
```
...
- TurnContext model exists at `ruche/pipelines/focal/models/turn_context.py`
...
- GlossaryItem model exists at `ruche/pipelines/focal/models/glossary.py`
...
- Template path: `ruche/pipelines/focal/context/prompts/situational_sensor.jinja2`
- Config model: SituationalSensorConfig in `ruche/config/models/pipeline.py`

## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/context/ -v
uv run pytest tests/unit/pipelines/focal/models/test_situational_snapshot.py -v
uv run pytest --cov=ruche/pipelines/focal/context --cov-report=term-missing
```
...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/retrieval/ -v
uv run pytest --cov=ruche/pipelines/focal/retrieval --cov-report=term-missing
```
...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/filtering/ -v
uv run pytest --cov=ruche/pipelines/focal/filtering --cov-report=term-missing
```
...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/filtering/test_scenario_filter.py -v
uv run pytest tests/unit/pipelines/focal/models/test_scenario_*.py -v
uv run pytest --cov=ruche/pipelines/focal/filtering --cov-report=term-missing
```
...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/execution/ -v
uv run pytest --cov=ruche/pipelines/focal/execution --cov-report=term-missing
```
...
- ResponsePlan goes to `ruche/pipelines/focal/planning/models.py`
- ResponsePlanner goes to `ruche/pipelines/focal/planning/planner.py`

## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/planning/ -v
uv run pytest --cov=ruche/pipelines/focal/planning --cov-report=term-missing
```
...
## Testing Commands
```bash
uv run pytest tests/unit/pipelines/focal/generation/ -v
uv run pytest tests/unit/pipelines/focal/enforcement/ -v
uv run pytest tests/unit/pipelines/focal/persistence/ -v
uv run pytest --cov=ruche/pipelines/focal --cov-report=term-missing
```
...
```bash
# 1. Ruff linting
uv run ruff check ruche/pipelines/focal/
uv run ruff check --fix ruche/pipelines/focal/  # Auto-fix issues

# 2. Ruff formatting
uv run ruff format ruche/pipelines/focal/

# 3. Mypy type checking
uv run mypy ruche/pipelines/focal/ --ignore-missing-imports

# 4. Tests
uv run pytest tests/unit/pipelines/focal/ -v --tb=short
```
...
uv run ruff check ruche/pipelines/focal/ && \
uv run ruff format --check ruche/pipelines/focal/ && \
uv run mypy ruche/pipelines/focal/ --ignore-missing-imports && \
uv run pytest tests/unit/pipelines/focal/ -v --tb=short && \
...
uv run pytest tests/integration/pipelines/focal/ -v
...
```bash
uv run pytest --cov=ruche/pipelines/focal --cov-report=html
...
```bash
# Full quality check on entire pipelines/focal module
uv run ruff check ruche/pipelines/focal/ && \
uv run ruff format --check ruche/pipelines/focal/ && \
uv run mypy ruche/pipelines/focal/ --ignore-missing-imports && \
uv run pytest tests/unit/pipelines/focal/ --cov=ruche/pipelines/focal --cov-fail-under=85
```
