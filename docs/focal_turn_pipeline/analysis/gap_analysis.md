...
|------|--------|----------|--------|--------|
| **Situational Sensor** | ✓ `[pipeline.situational_sensor]` | ✗ Uses `.txt` | ✗ None | Hardcoded fallback |
| **Rule Filter** | ✓ `[pipeline.rule_filtering]` | ✗ Uses `.txt` | ✗ None | Hardcoded fallback |
| **Scenario Filter** | ✓ `[pipeline.scenario_filtering]` | ✗ `.txt` unused | ✗ None | Deterministic only |
| **Response Generation** | ✓ `[pipeline.generation]` | ✗ Uses `.txt` | ✗ None | `str.format()` |
| **LLM Judge** | ✓ `[pipeline.enforcement]` | ✗ Missing | ✗ None | **Not implemented** |
...
**STATUS: PHASE 8 IS ENTIRELY SKIPPED** - Pipeline jumps from P6 (Scenario Orchestration) directly to P9 (Generation).
...
**Current Pipeline Flow** (from `engine.py`):
```
...
See `focal_turn_pipeline.md` Section 6 for the full Pipeline Execution Model.
...
The pipeline currently skips from Phase 6 → Phase 9, missing the Response Planning phase entirely. This means:
