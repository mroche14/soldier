...
- [ ] **Add enforcement_expression field to Rule model**
  - File: `focal/mechanics/focal/models/rule.py`
...
- [ ] **Add enforcement_expression to ConfigStore queries**
  - File: `focal/mechanics/focal/stores/config_store.py`
...
- [ ] **Create variable extraction service**
  - File: `focal/mechanics/focal/enforcement/variable_extractor.py`
  - Action: Create new file
  - Details:
    - Class: `VariableExtractor`
    - Method: `extract_variables(response: str, session: Session, profile: InterlocutorDataStore | None) -> dict[str, Any]`
    - Implements regex extraction for common patterns (amounts, percentages, flags)
    - Optional: LLM extraction for complex cases
  - Dependencies:
    - `re` (stdlib)
    - `focal.runtime.models.session.Session`
    - `focal.domain.interlocutor.models.InterlocutorDataStore`
...
- [ ] **Create deterministic enforcement service**
  - File: `focal/mechanics/focal/enforcement/deterministic_enforcer.py`
  - Action: Create new file
  - Details:
...
- [ ] **Create LLM-based enforcement service**
  - File: `focal/mechanics/focal/enforcement/subjective_enforcer.py`
  - Action: Create new file
  - Details:
...
- [ ] **Create Jinja2 prompt template**
  - File: `focal/mechanics/focal/enforcement/prompts/judge_rule_compliance.jinja2`
  - Action: Create new file
  - Details:
    ```jinja2
...
- [ ] **Add new dependencies to constructor**
  - File: `focal/mechanics/focal/enforcement/validator.py`
  ...
  - Details:
    ...
    - Add: `profile: InterlocutorDataStore | None` (pass to validate method)
...
- [ ] **Create relevance checking service**
  - File: `focal/mechanics/focal/enforcement/relevance_verifier.py`
...
- [ ] **Create grounding checking service**
  - File: `focal/mechanics/focal/enforcement/grounding_verifier.py`
  - Action: Create new file (optional feature)
  - Details:
...
## 9. FocalCognitivePipeline Integration
...
- [ ] **Pass session and profile to validator**
  ...
  - Details:
    ...
    - Add `profile` parameter (load from InterlocutorDataStoreInterface if needed)
...
### Existing Dependencies
  ...
  - `focal.infrastructure.providers.llm.LLMExecutor` - For LLM-as-Judge (Lane 2)
  - `focal.mechanics.focal.stores.config_store.ConfigStore` - Fetch GLOBAL rules
  - `focal.runtime.models.session.Session` - Session variables
  - `focal.domain.interlocutor.models.InterlocutorDataStore` - Customer fields
