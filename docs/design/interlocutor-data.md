...
> **Terminology note:** Older docs use `CustomerProfile` / `ProfileStore` / `ProfileField`. These names are deprecated. The canonical names are `InterlocutorDataStore` / `InterlocutorDataStoreInterface` / `VariableEntry`.
...
### InterlocutorDataStore Model

```python
class InterlocutorDataStore(BaseModel):
    """Persistent store of verified facts about a customer.

    Stored via InterlocutorDataStoreInterface. Implementations:
    - PostgresInterlocutorDataStore (relational)
    - InMemoryInterlocutorDataStore (testing)
    - InterlocutorDataStoreCacheLayer (cache wrapper)
...
Scenarios can **declare required fields** that should be persisted to `InterlocutorDataStore`. This creates a schema of expected customer data.

```python
class InterlocutorDataField(AgentScopedModel):
...
class ScenarioFieldRequirement(BaseModel):
    ...
    field_name: str                    # References InterlocutorDataField.name
...
class ScenarioStep(BaseModel):
    ...
    # If true, collected fields are persisted to InterlocutorDataStore
...
```python
class InterlocutorDataStoreInterface(ABC):
...
When a session starts, it links to a `customer_id` that resolves to an `InterlocutorDataStore`:
...
async def initialize_session(
    ...
    """Initialize a session, linking to InterlocutorDataStore."""

    # Get or create customer data
    interlocutor_data = await interlocutor_data_store.get_or_create(
        tenant_id=tenant_id,
        channel=channel,
...
The InterlocutorDataStore simplifies scenario entry and migration:
...
async def check_scenario_entry(
    ...
    if best.score >= config.entry_threshold:
        ...
        missing_fields = check_required_fields(scenario, interlocutor_data)
        ...
        if missing_fields and scenario.entry_requires_all_fields:
            ...
            return ScenarioFilterResult(
                ...
                missing_interlocutor_data_fields=missing_fields,  # New field
            )
...
async def evaluate_step_transition(
    ...
    for field_name in current_step.required_profile_fields:
        field = interlocutor_data.fields.get(field_name)
...
With InterlocutorDataStore, scenario migrations (from the scenario-update-methods.md) become simpler:

### Gap Fill Becomes Trivial

**Before**: LLM scans conversation history to extract missing values
**After**: Check InterlocutorDataStore for the required field
...
async def evaluate_new_fork(
    ...
    age = interlocutor_data.fields.get("date_of_birth")
...
```python
async def export_interlocutor_data(profile_id: UUID) -> InterlocutorDataExport:
    """Export all data for a customer (GDPR data portability)."""
    pass

async def delete_interlocutor_data(profile_id: UUID) -> None:
    """Delete all customer data (GDPR right to erasure)."""
    pass

async def anonymize_interlocutor_data(profile_id: UUID) -> None:
...
The `InterlocutorDataSchemaExtractor` uses an LLM to automatically identify customer data requirements:

```python
extractor = InterlocutorDataSchemaExtractor(llm_executor=llm)
...
- `focal_interlocutor_data_field_status` - Gauge of fields by status
- `focal_schema_extraction_success_total` - Extraction successes
- `focal_interlocutor_data_cache_hits_total` - Cache performance

Structured log events:
- `interlocutor_data_field_superseded` - When a field is replaced
- `interlocutor_data_field_expired` - When a field expires
- `interlocutor_data_field_orphaned` - When a field becomes orphaned
...
- [Scenario Update Methods](./scenario-update-methods.md) - How InterlocutorDataStore enables migrations
