...
The FOCAL 360 document proposes 9 major platform additions around the existing 11-phase turn pipeline. This analysis reveals:
...
**Channel Identity** (`ruche/customer_data/models.py`):
...
| **Agent Overrides**    | `AgentConfig`         | PARTIAL         | Model exists, not integrated into pipeline |
| **Scenario Overrides** | —                     | NOT IMPLEMENTED | No per-scenario config model               |
| **Step Overrides**     | `[pipeline.{step}]`   | IMPLEMENTED     | Per-step config in TOML                    |
...
**Status**: Defined but not wired into pipeline config resolution.
...
1. **Integrate AgentConfig**
   ...
   - Merge agent overrides onto pipeline defaults
...
| **CustomerDataStore** | `CustomerDataStoreInterface` | IMPLEMENTED | 457 methods, lineage tracking       |
...
1. **Integrate AgentConfig into Pipeline** (2-3 days)
...
| Topic                   | Document                                                  |
| ----------------------- | --------------------------------------------------------- |
| 11-Phase Pipeline       | `docs/focal_turn_pipeline/README.md`                      |
