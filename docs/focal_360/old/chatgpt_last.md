...
[LogicalTurnWorkflow] (Hatchet, owned by ACF)
    ↓
[AgentRuntime] → [AgentContext] (Agent + CognitivePipeline + Toolbox + channels)
    ↓
[CognitivePipeline.run/plan/finalize]  (FOCAL, LangGraph wrapper, Agno, etc.)
    ↓
...
* **AgentRuntime**
  ...
  * building & caching `AgentContext` (pipeline + toolbox + channel bindings),
...
2. **Thin waist**
   ...
   * Many pipelines on top (FOCAL, LangGraph, Agno).
   * Many channels below (WhatsApp, webchat, email), unified by ACF’s `RawMessage` / `OutboundMessage`.

3. **Pipeline autonomy**

   * Pipelines can implement complex mechanics (multi-step plans, retry loops, confirmations, agendas, etc.) **without** changing ACF.
   * ACF gives signals (pending messages, events), pipelines decide how to use them.

4. **Supersede ≠ ACF policy**

   * ACF detects new messages during a turn.
   * Pipeline decides: supersede, absorb, queue, or ignore.
...
6. **Tool control stays near the agent**
   ...
   * Tool choice, side-effect policy, and confirmation logic live in the CognitivePipeline + Toolbox, not in ACF.
...
class LogicalTurn(BaseModel):
    ...
    # Optional: references to artifacts (retrieval, rerank etc.) – pipeline-specific.
```
...
These are what pipelines return; ACF doesn’t care what happens inside.
...
class PipelineResult(BaseModel):
    ...
    # Pipeline may also include pipeline-specific artifacts.
...
class AgentContext:
    agent: "Agent"                     # config
    pipeline: "CognitivePipeline"      # FOCAL / LangGraph / Agno wrapper
    toolbox: "Toolbox"
...
* Tools executed **inside** `pipeline.run(ctx)` via `ctx.toolbox.execute()`.
* ACF doesn’t know about tools; it just runs `run()`.
* Supersede control via `await ctx.has_pending_messages()` inside pipeline, especially before irreversible tools.
...
* Requires pipeline to implement `plan` and `finalize`.
...
* can mark the current LogicalTurn as **superseded** and start a new one if the pipeline decides so.

ACF does **not** decide policy. It just:

* **applies** decisions coming from the pipeline (via the returned `PipelineResult`).

### 8.2 What the pipeline decides

Inside pipeline, before high-risk operations, you can do:
...
All the **semantics** (when to supersede, how to merge messages) stay in the pipeline.
...
class Agent(BaseModel):
    ...
    # scenario/rules config for FOCAL, or paths/settings for other pipelines
...
* **not part of ACF**,
* not part of the CognitivePipeline.
...
* Pipelines can emit custom events via `ctx.emit_event()`.
...
onto the FabricEvent stream without impacting pipeline code.
...
Because CognitivePipeline is only:
...
| **AgentContext**      | Pipeline, Toolbox, channel bindings                                                    | Infrastructure state                              |
| **AgentTurnContext**  | Connects FabricTurnContext to AgentContext                                             | Long-lived cached state                           |
| **CognitivePipeline** | Alignment & reasoning, when/which tools to call, supersede decisions, confirmations    | Mutex, workflows, external APIs                   |
...
| **ChannelGateway**    | Normalize inbound, format outbound, per-channel protocols                              | Agent selection & pipeline logic                  |
...
* or add a **shorter dev-facing “How to implement a new pipeline”** guide on top of this.
