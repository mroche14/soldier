...
1. **ACF vs CognitivePipeline boundary** - Clear separation of mechanics (ACF) from meaning (Pipeline)
2. **Four-state SupersedeDecision** - SUPERSEDE, ABSORB, QUEUE, FORCE_COMPLETE
3. **Two-phase commit for tools** - Pipeline proposes, ACF authorizes
4. **Pipeline-declared artifact reuse** - Pipeline knows semantics, ACF enforces fingerprints
...
- **Two-phase commit** - ACF authorizes tool execution, not just Pipeline
...
```
Pipeline proposes PlannedToolExecution[] → ACF validates → ACF authorizes → ToolHub executes → ACF records
```
...
| **CustomerDataStore** | "IMPLEMENTED" | **STILL RELEVANT** | Unchanged |
...
| 1.6 | CheckpointReuse (pipeline-declared) | Optimization | 05 |
...
| Priority | Component | Pipeline Role | Topic |
|----------|-----------|---------------|-------|
...
- `docs/focal_360/architecture/ACF_SPEC.md` - **Authoritative** implementation specification
- `docs/focal_360/architecture/LOGICAL_TURN_VISION.md` - Founding vision
- `docs/focal_360/architecture/README.md` - Index
- `docs/focal_360/architecture/topics/*.md` - Detailed specifications (with ACF context)
...
| Checkpoint Reuse | Pipeline-Declared Reuse | Pipeline declares policy, ACF enforces |
...
2. Pipeline makes semantic decision IF implements `SupersedeCapable`
...
- Capture BEFORE pipeline runs, after mutex acquired
...
- `FocalCognitivePipeline` adapter translates between them
