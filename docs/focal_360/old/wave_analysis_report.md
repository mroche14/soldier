...
2. **AgentConfig Integration**: Wire existing `AgentConfig` model into pipeline
...
1. **Rapid messages are legitimate user behavior**
   ...
   - Coalescing destroys turn-by-turn context the pipeline depends on
...
**What Needs Fixing:**
- Too many detection layers (middleware AND pipeline)
- Unclear thresholds
...
- Which config wins for shared pipeline steps?
...
**Hot Reload (Wide):**
- Touches every phase of pipeline
...
**Platform Immaturity:**
- Core 11-phase pipeline is 93% complete but not battle-tested
...
- `docs/focal_360/gap_analysis.md` - Existing vs missing
- `docs/focal_360/WAVE_EXECUTION_GUIDE.md` - Wave execution plans
