...
- Lives in ACF (shared across all pipelines)
...
> User confirmations are **tools** from the pipeline's point of view, decided by scenarios—not "magic" in ACF or channel adapters.
...
# Pipeline (P7.4) outputs PlannedToolExecution
...
|----------|-------------|
| "We need user confirmation" | CognitivePipeline (scenario step) |
...
1. **CognitivePipeline** decides *what* to do (confirm, ask, inform)
