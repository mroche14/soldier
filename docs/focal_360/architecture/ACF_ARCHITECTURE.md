...
│  │   - pipeline: CognitivePipeline (FOCAL, LangGraph, Agno)           │  │
...
│  │   emit_event: Callable[[FabricEvent], Awaitable[None]]               │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────────────────────┘
                                     │
                                     ▼
...
|-------|------|--------------|
| **ACF** | Mutex, turns, accumulation, supersede signals, Hatchet orchestration, FabricEvent routing | Tool execution, tool metadata, channel UX, business logic |
| **AgentRuntime** | Agent lifecycle, caching, invalidation | Turn processing, tool calls |
| **AgentContext** | Pipeline, Toolbox, ChannelBindings (configuration) | Runtime turn state |
...
1. **Session Mutex**
   ...
   - Guarantee: Never run two pipelines in parallel for the same session
...
2. **Turn Aggregation (TurnManager)**
   ...
   - Use time windows, typing indicators, or pipeline hints
...
3. **Supersede Signals (SupersedeCoordinator)**
   ...
   - Signal: Provide fact to Pipeline, not make decision
   - Pipeline decides SUPERSEDE/ABSORB/QUEUE/FORCE_COMPLETE
...
5. **FabricEvent Routing**
   - Receive events from Agent/Pipeline/Toolbox
...
These belong to **Agent** (Pipeline + Toolbox).
...
class FabricTurnContext(Protocol):
    ...
    async def has_pending_messages(self) -> bool:
        ...
        This is a FACT query. Pipeline decides what to do with the answer.
...
- **Brain**: CognitivePipeline (FOCAL, LangGraph, Agno)
- **Tools**: Toolbox (what tools it can use)
- **Channels**: ChannelBindings (where it's reachable)
...
class AgentContext:
    ...
    pipeline: CognitivePipeline            # FOCAL, LangGraph, Agno, etc.
...
class AgentTurnContext:
    """
    Per-turn context passed to Pipeline.
...
class CognitivePipeline(Protocol):
    ...
    ACF doesn't care which implementation; it just calls run().
    """
...
### 4.2 Tool Execution in Pipeline

Pipeline executes tools via Toolbox during turn processing:
...
Toolbox is the enforcement boundary - Pipeline trusts it to handle correctness.
...
- Pipeline may need response delays
...
- **Pipeline**: `policy = agent_ctx.channel_policies[channel]` → use `natural_response_delay_ms`
...
**Note**: All pipelines are conversational. For scheduled tasks, see §3.5 (TaskWorkflow bypasses pipelines entirely).
...
class Toolbox:
    ...
    - Records side effects via FabricEvents
    ...
    async def execute(
        ...
        3. Record side effect via FabricEvent
        4. Return result
        """
        ...
        # Record side effect via FabricEvent
...
class AGUIWebchatAdapter(ChannelAdapter):
    ...
    async def send(self, message: OutboundMessage) -> None:
        ...
        # Map FabricEvents to AG-UI event types
        ...
```
...
## 7. FabricEvents: Universal Event Model
...
class FabricEventType(str, Enum):
    ...
    # Pipeline status (Pipeline emits)
...
```
Pipeline/Toolbox → emit_event() → ACF FabricTurnContext
                                        │
                                        ▼
                                  ACF EventRouter
                                        │
...
async def run_pipeline(self, ctx: Context) -> dict:
    ...
    turn_ctx = self._build_turn_context(ctx, agent_ctx)

    # Pipeline runs P1-P11 with tool calls inside
    # Tools execute via ctx.toolbox.execute() - Toolbox enforces correctness
...
- Pipeline can't bypass policy checks (Toolbox enforces)
- Pipeline can't skip audit events (Toolbox emits them)
- Pipeline can't execute without idempotency (ToolGateway handles it)

### 8.3 Pipeline Conformance Requirements
...
2. ACF ORCHESTRATION
   ...
   Step 3: run_pipeline (ACF calls Agent)

3. AGENT EXECUTION
   AgentRuntime.get_or_create() → AgentContext
   Build AgentTurnContext(fabric + agent_context)

   AgentContext.pipeline.run(AgentTurnContext):
     ...
     P7: Tools via ctx.toolbox.execute()
         ...
         ← ToolResult
         → emit FabricEvent (TOOL_SIDE_EFFECT_COMPLETED)
...
2. **Agent is business entity**: Pipeline + Toolbox + Channels. Owns all decisions.

3. **Toolbox is the enforcement boundary**: Policy, idempotency, confirmation, audit. Pipeline calls it.

4. **Supersede = Facts + Decisions**: ACF provides facts, Pipeline decides actions.

5. **Events are the glue**: FabricEvents flow from Pipeline/Toolbox → ACF → everywhere.

6. **Single execution style**: Pipeline executes tools inline via Toolbox. No mode complexity.

7. **Channels are adapters**: AG-UI is just one webchat adapter, not ACF concern.

8. **Handoffs are mutations**: Pipeline proposes, ACF executes session transfer.

9. **Pipeline conformance via Toolbox + ASA**: Not runtime mode enforcement.
