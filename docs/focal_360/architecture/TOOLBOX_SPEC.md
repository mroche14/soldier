...
The **Toolbox** layer handles tool execution for Agents. It bridges the business layer (Agent/Pipeline) with the infrastructure layer (ToolGateway/Providers).
...
| **Tier 3** | **Agent-Enabled** | Tools this agent can use | Toolbox, Pipeline (for execution) |
...
class Toolbox:
    ...
    - Record side effects via FabricEvents
    ...
    async def execute(
        ...
        4. Record side effect via FabricEvent
        5. Return result
        """
        ...
        await turn_context.emit_event(FabricEvent(
            type=FabricEventType.TOOL_SIDE_EFFECT_STARTED,
        ...
        try:
            ...
            # Emit failure event
            await turn_context.emit_event(FabricEvent(
                type=FabricEventType.TOOL_SIDE_EFFECT_FAILED,
                tenant_id=exec_ctx.tenant_id,
                agent_id=exec_ctx.agent_id,
                session_key=turn_context.session_key,
                turn_id=turn_context.logical_turn.id,
    ...
    def get_metadata(self, tool_name: str) -> ToolMetadata | None:
        ...
        Used by Pipeline to decide supersede behavior before execution.
        """
...
class PlannedToolExecution(BaseModel):
    """
    Pipeline's proposal to execute a tool.

    Created by Pipeline during planning, executed by Toolbox.
    """
...
class SideEffectRecord(BaseModel):
    ...
    Emitted via FabricEvent; ACF stores in LogicalTurn.side_effects.
    """
...
# Pipeline executes tool directly
...
class FocalCognitivePipeline(CognitivePipeline):
    """FOCAL pipeline with supersede-aware tool execution."""
...
```python
class FocalCognitivePipeline(CognitivePipeline):
    """Pipeline with confirmation support."""
