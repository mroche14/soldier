...
| **Message absorption** triggers re-run | Pipeline may reach same tool call again |
...
- **Artifact reuse**: Cached pipeline outputs are validated before reuse
...
**Purpose**: If inputs are identical, cached pipeline artifacts *might* be reusable.
...
Pipeline-owned policy declaring what can be reused and when:
...
**Purpose**: Keep domain knowledge in the Pipeline. ACF enforces, Pipeline declares.
...
Timeline:
  ...
  T+800ms  Pipeline reaches refund tool (first time in restarted turn)
...
Timeline:
  ...
  T+500ms  Message 2 arrives → ACF decides ABSORB
  T+600ms  Turn A restarts from checkpoint with both messages
  T+800ms  Pipeline proposes refund tool with idempotency key:
...
| **`reuse_decl`** | Pipeline's explicit declaration of what artifacts are safe to reuse under which conditions |
...
- [05-checkpoint-reuse.md](05-checkpoint-reuse.md) - Artifact reuse, ReusePolicy (ACF/Pipeline boundary)
