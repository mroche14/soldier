...
> **Topic**: Preventing concurrent pipeline execution per conversation
...
| Lock Extension | ACF | Extended during long pipeline runs |
...
The **Session Mutex** enforces a single-writer rule: no two pipeline executions can run concurrently for the same conversation.
...
async def acquire_mutex(self, ctx: Context) -> dict:
    ...
    # Emit ACF event
    await emit_fabric_event(FabricEventType.TURN_STARTED, ...)
