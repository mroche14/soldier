...
> **Dependencies**: CognitivePipeline interface, Side-Effect Policy (for alignment mechanic)
...
**ASA (Agent Setter Agent)** is a **mechanic-agnostic meta-agent** that helps design, validate, and configure ANY CognitivePipeline implementation. It is NOT specific to FOCAL's alignment mechanic.

### Key Capabilities

1. **Universal Conformance Testing** - Every CognitivePipeline must pass ASA's conformance suite (tools via Toolbox, event emission, supersede checks, timeout handling, tenant isolation)
...
| FOCAL-specific scenario validator | Mechanic-agnostic pipeline configurator |
...
- Runs conformance tests against any CognitivePipeline implementation

---

## Pipeline Conformance Tests

**Every CognitivePipeline implementation must pass ASA's conformance test suite**, regardless of which mechanic it implements.
...
class PipelineConformanceTests:
    ...
    async def test_tools_go_through_toolbox(self, pipeline: CognitivePipeline, ctx: TurnContext):
    ...
    async def test_required_events_are_emitted(self, pipeline: CognitivePipeline, ctx: TurnContext):
    ...
    async def test_supersede_checked_before_irreversible(
        ...
        assert result.superseded, "Pipeline must detect supersede"
        assert len(result.tool_calls) == 0, \
            "No irreversible tools should execute when superseded"

    async def test_pipelineresult_contract(self, pipeline: CognitivePipeline, ctx: TurnContext):
        ...
        assert result.metadata.get("pipeline_version") is not None, \
            "Must declare pipeline version"

    async def test_timeout_handling(self, pipeline: CognitivePipeline, ctx: TurnContext):
        ...
        assert duration_ms < 200, "Pipeline must respect timeout"
        assert result.timed_out, "Result should indicate timeout"

    async def test_tenant_isolation(self, pipeline: CognitivePipeline, ctx: TurnContext):
...
class AlignmentMechanicConformance(PipelineConformanceTests):
    ...
    async def test_checkpoint_enforcement(self, pipeline: AlignmentPipeline, ctx: TurnContext):
        """Alignment mechanic MUST enforce checkpoint steps before irreversible actions."""
        # ... mechanic-specific tests

    async def test_scenario_step_tracking(self, pipeline: AlignmentPipeline, ctx: TurnContext):
...
class ReactMechanicConformance(PipelineConformanceTests):
    ...
    async def test_reasoning_trace_captured(self, pipeline: ReactPipeline, ctx: TurnContext):
        """ReAct mechanic MUST capture reasoning traces."""
        # ... mechanic-specific tests

    async def test_max_iterations_enforced(self, pipeline: ReactPipeline, ctx: TurnContext):
...
```bash
# Test a specific pipeline implementation
...
focal asa conformance-report --pipeline alignment --output report.json
```
...
## ASA as Pipeline Configurator

ASA can configure **any CognitivePipeline implementation**, not just alignment-based ones.
...
class CognitivePipeline(ABC):
    ...
    @abstractmethod
    async def process_turn(self, ctx: TurnContext) -> PipelineResult:
...
**Alignment Mechanic:**
```python
class AlignmentPipeline(CognitivePipeline):
    @classmethod
    def get_config_schema(cls) -> ConfigSchema:
        return ConfigSchema(
            artifacts=[
...
**ReAct Mechanic:**
```python
class ReactPipeline(CognitivePipeline):
    @classmethod
    def get_config_schema(cls) -> ConfigSchema:
        return ConfigSchema(
            artifacts=[
...
```python
class PlannerExecutorPipeline(CognitivePipeline):
    @classmethod
    def get_config_schema(cls) -> ConfigSchema:
        return ConfigSchema(
            artifacts=[
...
class CustomQAMechanic(CognitivePipeline):
...
- [12-cognitive-pipeline-interface.md](12-cognitive-pipeline-interface.md) - Base interface ASA validates against
