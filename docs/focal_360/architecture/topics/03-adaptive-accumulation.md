...
Adaptive Accumulation is an **ACF component** that determines when a LogicalTurn is ready for processing. ACF owns the accumulation loop; CognitivePipeline may provide hints that influence timing.
...
| Plan Hints | CognitivePipeline | Pipeline suggests expected inputs |

### Pipeline Hints (Previous-Turn Storage Pattern)

**Key insight**: Accumulation hints come from the PREVIOUS turn, not the current pipeline execution (no circular dependency).

**How it works**:
1. Turn N completes, pipeline returns `PipelineResult.accumulation_hint`
...
4. Uses hint to extend window if pipeline asked a question
...
When pipeline is mid-scenario and has asked the user a question, it signals `awaiting_required_field=True`, and ACF extends the accumulation window on the NEXT turn.
...
[pipeline.accumulation.channels]
whatsapp = 1200
telegram = 1000
sms = 800
web = 600
...
[pipeline.accumulation.prediction]
enabled = false
model = "haiku"
...
async def accumulate(self, ctx: Context) -> dict:
    ...
    Loads hints from PREVIOUS turn's pipeline result.
    """
...
session.last_pipeline_result = LastPipelineResult(
    turn_id=turn.id,
