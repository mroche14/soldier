...
- `AgentContext` - Runtime instance (Pipeline + Toolbox + Channels)
...
class Agent(BaseModel):
    ...
    # Pipeline type (determines which CognitivePipeline to use)
    pipeline_type: str = "focal"  # "focal", "langgraph", "agno"

    # Pipeline-specific configuration (scenarios/rules for FOCAL, graph config for LangGraph, etc.)
...
`AgentTurnContext` wraps ACF's `FabricTurnContext` with the Agent's runtime components. This is what gets passed to the Pipeline.

```python
@dataclass
class AgentTurnContext:
    """
    Per-turn context passed to CognitivePipeline.

    Wraps FabricTurnContext (ACF infrastructure) with AgentContext (business).
    Pipeline uses this to access everything it needs for turn processing.
    """
    ...
    async def has_pending_messages(self) -> bool:
        ...
        Pipeline uses this to decide supersede behavior before irreversible tools.
        """
    ...
    async def emit_event(self, event: FabricEvent) -> None:
        ...
        Pipeline/Toolbox emit events; ACF routes to listeners.
        """
...
class AgentRuntime:
    ...
    async def _load_channel_policies(
        ...
        used by ACF (accumulation), Agent (pipeline), and ChannelGateway (formatting).
        """
...
Creates CognitivePipeline instances based on agent configuration:

```python
class PipelineFactory:
    """
    Factory for creating CognitivePipeline instances.

    Supports: FOCAL, LangGraph, Agno, custom pipelines.
    """

    def __init__(
        self,
        focal_factory: Callable[[Agent], CognitivePipeline] | None = None,
        langgraph_factory: Callable[[Agent], CognitivePipeline] | None = None,
        agno_factory: Callable[[Agent], CognitivePipeline] | None = None,
    ):
        self._factories = {
            "focal": focal_factory,
    ...
    def register(
        self,
        pipeline_type: str,
        factory: Callable[[Agent], CognitivePipeline],
    ) -> None:
        """Register custom pipeline factory."""
...
class PipelineTypeNotFoundError(Exception):
    ...
    def __init__(self, pipeline_type: str):
        ...
        super().__init__(f"Unknown pipeline type: {pipeline_type}")
```
