...
> ACF is the **conversation control plane** that governs *when and how an agent responds*, separate from *what the agent says* (Brain).

### Execution Model

FOCAL uses a **single execution style**: Brain calls `toolbox.execute()` inline.
...
- Brain conformance via Toolbox + ASA validation, not runtime modes
...
1. **Read the ACF specification**: `docs/acf/architecture/ACF_SPEC.md`
2. **Read the founding vision**: `docs/acf/architecture/LOGICAL_TURN_VISION.md`
...
| Adaptive Accumulation + Brain Hints | Aggregation | `ruche/alignment/gateway/accumulator.py` | Hints from PREVIOUS turn |
...
1. `docs/acf/architecture/ACF_SPEC.md` - THE AUTHORITATIVE SPECIFICATION
2. `docs/acf/architecture/ACF_ARCHITECTURE.md` - Canonical architecture (v3.0)
...
ACF owns the LogicalTurn lifecycle; Brain operates on it to produce meaning.
...
class LogicalTurnStatus(str, Enum):
    ...
    PROCESSING = "processing"       # Brain running
...
class SupersedeAction(str, Enum):
    ...
    When a new message arrives during processing, Brain
...
class PhaseArtifact(BaseModel):
    """Cached result from a brain phase for checkpoint reuse."""
...
## CRITICAL: Read These Files FIRST (in order)
1. `docs/acf/architecture/ACF_SPEC.md` - THE AUTHORITATIVE SPECIFICATION
2. `docs/acf/architecture/TOOLBOX_SPEC.md` - Tool execution spec
3. `CLAUDE.md` - Project conventions
...
class TurnAction(str, Enum):
    ...
    PROCESS = "process"        # Start brain now
...
3. `run_pipeline` - Brain invocation (ACF ↔ Brain boundary)
...
class LogicalTurnWorkflow:
    ...
    async def run_pipeline(self, ctx: Context) -> dict:
        """
        Execute alignment brain with interrupt checks.
        """
...
4. Interrupt-aware brain execution
...
- [ ] Brain-declared artifact reuse working
...
### Agent 2B: Supersede-Aware Brain

**Prompt**:

```markdown
# Task: Make Brain Supersede-Aware
...
| 2 | SideEffectPolicy, Supersede-Aware Brain | 2 | Wave 0+1 |
