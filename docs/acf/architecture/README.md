...
> **Purpose**: Complete architectural documentation for the Agent Conversation Fabric
...
|-------|------|--------------|
| **ACF** | Mutex, turns, workflow, supersede signals, ACFEvent routing | Tool execution, tool semantics, business logic |
...
**Key principle**: Toolbox is the enforcement boundary. ACF is NOT in the tool execution path. Brain conformance is enforced via Toolbox + ASA validation, not runtime modes.
...
| 05 | [Checkpoint Reuse](topics/05-checkpoint-reuse.md) | Brain | Artifact reuse |
...
ACF provides **facts** (`has_pending_messages()`), Brain makes **decisions**.
...
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       ...
                                       ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                    COGNITIVE BRAIN (FOCAL/LangGraph/Agno)                  │
│  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐            │
