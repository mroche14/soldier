...
> The workflow calls AgentRuntime to get AgentContext, then delegates to Brain.
...
| Brain context | `FabricTurnContext` | `AgentTurnContext` wrapping `FabricTurnContext` |
| Agent loading | N/A | `AgentRuntime.get_or_create()` |
| Side effects | ACF SideEffectLedger | Toolbox emits ACFEvents → ACF stores |
...
| Brain execution | Step 3: `brain.think(AgentTurnContext)` |
| Tool execution | `ctx.toolbox.execute()` inside Brain (Toolbox enforces) |
...
| Supersede decision | Brain decides (SUPERSEDE/ABSORB/QUEUE/FORCE_COMPLETE) |
| ACFEvent stream | Workflow step outputs + external events |
...
class LogicalTurnWorkflow:
    ...
    - Brain invocation (Agent owns meaning)
    - ACFEvent routing (side effects stored here)
    - Supersede signals (ACF provides facts, Brain decides)
    """
...
class LogicalTurnWorkflow:
    ...
    async def commit_and_respond(self, ctx: Context) -> dict:
        ...
        Only runs if brain completed (not superseded).
        """
...
async def run_agent(self, ctx: Context) -> dict:
    """
    Agent's Brain runs all phases with tool execution inside.

    Tools execute INSIDE this step via ctx.toolbox.execute().
    Brain can check ctx.has_pending_messages() for supersede signals.
    ...
    # Brain runs all phases - tools via ctx.toolbox.execute()
