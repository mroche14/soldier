...
> **Impacts**: All brain phases, TurnRecord, Session management
...
LogicalTurn is the **central abstraction** in the Agent Conversation Fabric. ACF owns the LogicalTurn lifecycle (creation, accumulation, state transitions) while Brain operates on a LogicalTurn to produce meaning.
...
| Supersede Decisions | Brain (via ACF) | Brain advises, ACF enforces |
...
class LogicalTurnStatus(str, Enum):
    ...
    PROCESSING = "processing"       # Brain is running
...
class LogicalTurn(BaseModel):
    ...
    interrupt_point: str | None = None  # Where in brain when interrupted
    ...
    def can_absorb_message(self) -> bool:
        ...
        consult Brain via decide_supersede() for the full decision.
        ...
        if self.status == LogicalTurnStatus.PROCESSING:
            ...
            # Full decision requires consulting Brain
...
When a new message arrives during PROCESSING, ACF coordinates with Brain:
...
┌───────────────────────────────────────┐
│ ACF calls Brain.decide_supersede() │
│ - current: LogicalTurn                │
│ - new: RawMessage                     │
│ - interrupt_point: str                │
└───────────────────┬───────────────────┘
...
- [05-checkpoint-reuse.md](05-checkpoint-reuse.md) - PhaseArtifact model (Brain-declared)
