...
> ACF only stores side effect records received via ACFEvents.
...
| Side effect recording | Toolbox | Creates `SideEffectRecord`, emits ACFEvent |
...
| Supersede decision | Brain | Uses metadata + signal to decide |
...
issue_refund: IRREVERSIBLE
  ...
  → If pending messages, Brain decides SUPERSEDE/ABSORB/CONTINUE
...
class SideEffectPolicy(str, Enum):
    ...
    Brain uses this to decide supersede behavior.
    """
    ...
    Before execution:
    - Brain should check has_pending_messages()
...
Brain uses `Toolbox.get_metadata()` to check policy before execution:
...
class Toolbox:
    def get_metadata(self, tool_name: str) -> ToolMetadata | None:
        ...
        Called by Brain before execution to determine:
...
class SideEffectRecord(BaseModel):
    ...
    Stored by: ACF (via ACFEvent → TurnManager)
    """
...
## Brain Integration: Supersede Check

Brain checks `has_pending_messages()` before IRREVERSIBLE tools:

```python
class FocalBrain(Brain):
    """Brain with supersede-aware tool execution."""
...
When Toolbox executes a tool, it emits a ACFEvent that ACF stores:
...
# Supersede decisions (Brain emits)
...
```python
# Test: Brain checks supersede before IRREVERSIBLE
async def test_irreversible_checks_supersede(brain, mock_toolbox, mock_fabric_ctx):
    ...
    # Brain should check and handle supersede
    with pytest.raises(SupersededError):
        await brain._execute_tools([...], mock_turn_ctx)


# Test: PURE tools don't check supersede
async def test_pure_skips_supersede_check(brain, mock_toolbox, mock_fabric_ctx):
    mock_toolbox.get_metadata.return_value = ToolMetadata(
        name="get_status",
        side_effect_policy=SideEffectPolicy.PURE,
    )
...
# Test: Toolbox emits ACFEvent on execution
