...
> **Boundary**: Brain-declared, ACF-enforced
...
Checkpoint Reuse involves **shared responsibility** between ACF and Brain:
...
| **Reuse Policy Declaration** | **Brain** | Declares what's safe to reuse |
| **Artifact Data** | **Brain** | Produces artifact content |

### Key Insight: Brain Declares, ACF Enforces

ACF cannot know the semantics of phase outputs—only Brain knows whether P4 retrieval results are still valid if the intent changed. Therefore:

1. **Brain declares** ReusePolicy for each phase artifact
2. **ACF stores** artifacts with fingerprints
3. **On restart**, ACF asks Brain which artifacts to reuse
4. **ACF validates** fingerprints match

```python
class ReusePolicy(str, Enum):
    """Brain-declared reuse semantics."""
...
**Checkpoint Reuse** prevents re-running expensive brain phases when a turn is superseded. If the inputs to a phase haven't changed, reuse its cached output.
...
class PhaseArtifact(BaseModel):
    """
    Cached output from a brain phase.
...
class FingerprintComputer:
    ...
    def compute_phase_input_fingerprint(
        ...
        if phase == 1:  # Context Loading
            ...
            data = {
                ...
                "customer_data_hash": self._hash_interlocutor_data(context.interlocutor_data),
            }
    ...
    def _hash_interlocutor_data(self, data: InterlocutorDataStore) -> str:
        return hashlib.md5(
...
## Phase Reusability (Brain-Declared)

Brain declares reusability for each phase. This is **not hardcoded in ACF**—the brain knows its own semantics:
...
# Example: Brain declares reuse policies
# (This is brain code, not ACF code)
class FOCALPipeline(Brain):
...
2. On restart, checking fingerprints against brain declarations
...
async def restart_with_reuse(
    ...
    # Get brain's reuse declarations
    reuse_policies = brain.get_reuse_policies()
