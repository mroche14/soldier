...
**Supersede Signaling**: ACF detects new messages during processing. Brain decides action.
...
async def run_pipeline(self, ctx: Context) -> dict:
    ...
    result = await brain.run(turn_ctx)
...
class ACFEvent(BaseModel):
    type: ACFEventType  # TURN_STARTED, TOOL_SIDE_EFFECT_COMPLETED, etc.
    tenant_id: UUID
    agent_id: UUID
    session_key: str
...
# Brain waits for confirmation turn
...
**ACFEvents for Status**:

```python
# Toolbox emits events during execution
await ctx.emit_event(ACFEvent(
    type=ACFEventType.TOOL_EXECUTION_STARTED,
