...
- Explicit boundary between ACF (mechanics) and Brain (meaning)
- Four-state SupersedeDecision model
- Two-phase commit for tool side effects
- Brain-declared artifact reuse semantics
...
The legacy brain spec processed “a single user message through 11 phases.” With ACF, the brain runs **once per LogicalTurn** (one or more messages).
...
**Rule:** No two brain executions concurrently for the same logical conversation key:
...
|--------|-----------|---------------|---------------------------|---------|-------------------------|
| Plain brain | 1 message = 1 turn | Low | Medium | Great | N/A |
