...
### 3.5 Object Selection Brain

Every retrievable object type in Focal goes through a **unified selection brain** after initial vector/hybrid retrieval. This brain is configurable per object type and consists of two stages:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        OBJECT SELECTION BRAIN                             │
...
#### 3.5.1 Object Types Using This Brain
...
Each object type has its own reranking + selection configuration under `[brain.retrieval.*_selection]`.
...
Per-object-type configuration combining reranking + selection strategy (matches `[brain.retrieval.*_selection]`):
...
class OutcomeCategory(BaseModel):
    ...
    source: Literal["BRAIN", "GENERATION"]

    category: Literal[
        # Brain-determined (set by code, deterministic)
...
|----------|--------|--------|---------|
| `AWAITING_USER_INPUT` | `BRAIN` | Phase 8 (planning) | Need order ID to proceed |
| `SYSTEM_ERROR` | `BRAIN` | Phase 7 (tool execution) | Tool 'order_lookup' timed out |
| `POLICY_RESTRICTION` | `BRAIN` | Phase 10 (enforcement) | Rule 'refund_limit' blocked |
...
Phase 10 (Enforcement):
    ...
    │       source="BRAIN", category="POLICY_RESTRICTION",
...
Final TurnOutcome:
    resolution: PARTIAL
    categories: [
        {source: BRAIN, category: AWAITING_USER_INPUT, scenario_id: "kyc_flow"},
...
class TurnRecord(BaseModel):
    ...
    # Turn outcome for observability (categories accumulate through brain)
    outcome: TurnOutcome
...
- `categories`: List of `OutcomeCategory` entries accumulated during the brain
...
FROM turn_records tr,
     unnest(tr.outcome.categories) as oc
WHERE oc.category = 'SYSTEM_ERROR' AND oc.source = 'BRAIN'
GROUP BY 1 ORDER BY 2 DESC;
...
-- Brain vs Generation category distribution
