...
Currently, brain configuration is stored in **TOML files** and loaded at startup:
...
Each brain step can be individually configured. Model strings use provider prefixes (e.g., `openrouter/anthropic/claude-3-haiku`) and support OpenRouter provider routing.
...
Each object type has its own **reranking + selection strategy** configuration under `[brain.retrieval.*_selection]`.

See Section 3.5 for the unified selection brain concept.
