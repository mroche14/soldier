...
> **Note:** This brain runs **once per LogicalTurn** (one or more messages). Turn boundaries, message accumulation, and supersede signals are handled by ACF. See `docs/acf/architecture/ACF_SPEC.md`.
...
| P1.6 | Load static config                         | Load brain config, LLM tasks, InterlocutorDataFields, glossary items| `tenant_id`, `agent_id`                                                   | `PipelineConfig`, `LlmTaskConfig`s, `InterlocutorDataField`s, `GlossaryItem`s|
...
* uses a **masked** view of InterlocutorDataStore (what exists, not the values),
...
>
> The Intent Registry is an **observability layer**, not a runtime enforcement mechanism. It's documented separately from the core brain.
...
**Brain categories already set before Phase 9:**

By the time we reach generation, the brain has already appended:
