...
|---------------|---------------|-------|
| `ruche/alignment/` | `ruche/brains/focal/` | ~250 |
| `ruche.alignment.` | `ruche.brains.focal.` | ~50 |
...
| `ruche/interlocutor_data/` | `ruche/domain/interlocutor/` | ~150 |
| `ruche.interlocutor_data.` | `ruche.domain.interlocutor.` | ~30 |
| `AlignmentEngine` | `FocalBrain` | ~30 |
| `InterlocutorDataStore` | `InterlocutorDataStore` | ~80 |
| `InterlocutorDataField` | `InterlocutorDataField` | ~60 |
...
- File: `ruche/interlocutor_data/models.py`
...
+ **Add explicit customer resolution method to FocalBrain**
...
```bash
cd docs/focal_brain/implementation
for file in *.bak; do
  mv "$file" "${file%.bak}"
done
```
...
1. Use the updated paths in `ruche/brains/focal/` for alignment/cognitive pipeline code
...
5. Reference `FocalBrain` instead of `AlignmentEngine`
