...
> **Reference Documents:**
> - Primary: `docs/focal_brain/README.md` (Phase 1, lines 209-229)
> - Gap Analysis: `docs/focal_brain/analysis/gap_analysis.md` (lines 99-119)
...
- Static configuration (brain config, glossary, customer data schema)
...
- [x] **Create TurnContext model**
  ...
  - Details:
    ...
    class TurnContext(BaseModel):
        ...
        Loaded in Phase 1, used throughout brain.
        """
        # Routing
        tenant_id: UUID
        agent_id: UUID
        ...
        interlocutor_data_fields: dict[str, InterlocutorDataField]  # name -> definition
...
- [x] **Rename ProfileFieldDefinition → InterlocutorDataField**
  ...
  - Why: Aligns with focal brain spec naming
...
- [x] **Create InterlocutorSchemaMask model**
  ...
  - Details:
    ...
    def build_customer_schema_mask(
        interlocutor_data: InterlocutorDataStore,  # Renamed from CustomerProfile
...
### 2.3 Update Brain Configuration Model
...
- [x] **Add turn context config**
  ...
  - Details:
    ...
    class TurnContextConfig(BaseModel):
        ...
        load_interlocutor_data_schema: bool = Field(default=True)
...
- [x] **Add explicit customer resolution method to FocalBrain**
...
- [x] **Add InterlocutorDataStore loader**
  ...
  - Details: Created `ruche/brains/focal/loaders/interlocutor_data_loader.py`

- [x] **Implement InterlocutorDataLoader**
  - File: `ruche/brains/focal/loaders/interlocutor_data_loader.py`
  - Action: Created new file
  - Details:
    ```python
...
- [x] **Add config loader for glossary and schema**
  ...
  - Details:
    ...
    class StaticConfigLoader:
        ...
        async def load_interlocutor_data_schema(
            self,
            tenant_id: UUID,
            agent_id: UUID,
        ) -> dict[str, InterlocutorDataField]:
            """Load customer data field definitions."""
            fields = await self._config_store.get_interlocutor_data_fields(
                tenant_id=tenant_id,
                agent_id=agent_id,
                enabled_only=True,
            )
...
- [x] **Add build_turn_context method to FocalBrain**
  - File: `ruche/brains/focal/engine.py`
  - Action: Added private method
  ...
  - Details:
    ...
    async def _build_turn_context(
        ...
        if self._config.turn_context.load_interlocutor_data_schema:
            interlocutor_data_fields = await self._static_config_loader.load_interlocutor_data_schema(
                tenant_id=turn_input.tenant_id,
                agent_id=turn_input.agent_id,
            )
        ...
        return TurnContext(
            tenant_id=turn_input.tenant_id,
            agent_id=turn_input.agent_id,
            customer_id=interlocutor_data.customer_id,
            session_id=session.session_id,
            turn_number=session.turn_count + 1,
            session=session,
            interlocutor_data=interlocutor_data,
            pipeline_config=self._config,
            interlocutor_data_fields=interlocutor_data_fields,
            glossary=glossary,
...
- [x] **Tests exist in interlocutor_data module**: `tests/unit/interlocutor_data/test_interlocutor_data_models.py`
  - File: Would be `tests/unit/interlocutor_data/test_interlocutor_data_models.py`
...
- [x] **Tests exist in interlocutor_data module**: `tests/unit/interlocutor_data/test_interlocutor_data_models.py`
  - File: Tests exist in `tests/unit/interlocutor_data/`
...
- [x] **Test InterlocutorDataLoader**
  - File: `tests/unit/alignment/loaders/test_interlocutor_data_loader.py`
  - Action: Created new file
...
- [x] **Integration tests verified**
  ...
  - Tests covered:
    - Full turn processing brain with customer resolution ✓
...
- [x] **Timing metrics implemented**
  ...
  - **Future Enhancement**: Consider adding Prometheus counters/histograms in `ruche/observability/metrics.py`:
    ...
    - `interlocutor_data_field_count` - Histogram for field counts
...
- [x] **Phase 1 log events added**
  ...
  - Details:
    ...
    - Existing logs in `_build_turn_context` (lines 1372-1377, 1386-1391, 1403-1408):
      ...
      - `interlocutor_data_schema_load_failed` - when schema loading fails
...
- [x] **OpenTelemetry spans**: Infrastructure ready, spans can be added incrementally
  ...
  - OTel spans can be added when instrumenting full brain
...
- [x] **CLAUDE.md updated with Phase 1 documentation**
  - File: `CLAUDE.md`
  - Added: "FOCAL Brain (Phase 1 Implementation)" section
  ...
  - Documents: InterlocutorDataLoader, StaticConfigLoader
  - Documents: Usage in FocalBrain
...
- [x] **Implementation guide: This checklist serves as the guide**
  - File: `docs/focal_brain/implementation/phase-01-identification-checklist.md`
...
- Intent catalog (P4.2-P4.3 in focal_brain.md)
...
6. ✅ FocalBrain has explicit Phase 1 methods (P1.1-P1.8)
...
| Loaders (InterlocutorDataLoader, StaticConfigLoader) | 3 hours |
| ConfigStore interface extensions | 2 hours |
| InMemoryConfigStore implementation | 2 hours |
| FocalBrain refactoring | 4 hours |
| Unit tests | 6 hours |
...
4. **FocalBrain Integration** (Day 2 PM - Day 3)
...
- Tests: PASSING (130/130 interlocutor_data tests)
- Coverage: All interlocutor_data module tests passing
...
- `ruche/brains/focal/loaders/interlocutor_data_loader.py` - Updated imports
...
- `tests/integration/stores/test_postgres_interlocutor_data.py` - Updated imports
- `tests/contract/test_interlocutor_data_store_contract.py` - Updated imports
- `tests/performance/test_interlocutor_data_performance.py` - Updated imports
- `tests/unit/interlocutor_data/test_interlocutor_data_models.py` - Updated imports
- `tests/unit/interlocutor_data/stores/test_inmemory_interlocutor_data.py` - Updated imports
- `tests/unit/interlocutor_data/stores/test_cached_interlocutor_data.py` - Updated imports
- `tests/unit/interlocutor_data/test_validation.py` - Updated imports
- `tests/unit/interlocutor_data/test_extraction.py` - Updated imports
...
All interlocutor_data unit tests passing:
...
- `tests/unit/profile/` → `tests/unit/interlocutor_data/` ✓

**Test File Renames:**
- `test_profile_models.py` → `test_interlocutor_data_models.py` ✓
- `test_inmemory_profile.py` → `test_inmemory_interlocutor_data.py` ✓
- `test_cached_profile.py` → `test_cached_interlocutor_data.py` ✓
- `test_postgres_profile.py` → `test_postgres_interlocutor_data.py` ✓
- `test_profile_store_contract.py` → `test_interlocutor_data_store_contract.py` ✓
- `test_profile_performance.py` → `test_interlocutor_data_performance.py` ✓

**All Imports Updated:**
- 27 files updated from `ruche.profile` → `ruche.interlocutor_data`
...
- TurnContext available for future brain steps to use
- Graceful degradation when profile_store not available

**Next Steps:**
- Consider passing `turn_context` to brain steps instead of individual fields (future optimization)
