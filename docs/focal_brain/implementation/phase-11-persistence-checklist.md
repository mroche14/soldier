...
> **Reference**: `docs/focal_brain/README.md` (Phase 11), `docs/focal_brain/analysis/gap_analysis.md` (Parallelism)
...
- [ ] **Refactor FocalBrain persistence to use asyncio.gather**
  ...
  - New code:
    ...
    if interlocutor_data_updates and persist:
        persistence_tasks.append(
            self._persist_interlocutor_data(
                session=session,
                updates=interlocutor_data_updates,
            )
        )
...
- [ ] **Track parallel persistence duration**
  ...
  - Details: Add histogram for parallel persistence timing
    ...
    PERSISTENCE_DURATION = Histogram(
        ...
        ["operation"],  # session, interlocutor_data, turn_record
    )
    ```
...
- [ ] **Create _persist_interlocutor_data method**
  - File: `ruche/brains/focal/engine.py`
  - Action: Add
  - Details: Filter and persist only non-SESSION scoped fields
    ```python
    async def _persist_interlocutor_data(
        self,
        session: Session,
        ...
        if not persistent_updates:
            logger.debug(
                "no_interlocutor_data_to_persist",
...
- [ ] **Add InterlocutorDataUpdate model**
  - File: `ruche/brains/focal/models/interlocutor_data.py`
  - Action: Create
...
- [ ] **Add TaskQueue to FocalBrain constructor**
  - File: `ruche/brains/focal/engine.py`
  - Action: Modify
...
- [ ] **Create entity_extraction.jinja2 template**
  ...
  - Details: Extract the 192-line inline prompt from `entity_extractor.py:45-237`
    ...
    ## Customer Schema
    {% for field in interlocutor_data_fields %}
...
- [ ] **Refactor EntityExtractor to use Jinja2 template**
  ...
  - Details: Replace hardcoded prompt with template loader
    ...
    class EntityExtractor:
        ...
        async def extract(self, conversation: list, interlocutor_data_fields: list) -> list[Entity]:
...
- [x] `OutcomeCategory` model - `ruche/brains/focal/models/outcome.py`
- [x] `TurnOutcome` model - `ruche/brains/focal/models/outcome.py`
- [x] `build_turn_outcome()` helper - `ruche/brains/focal/generation/resolution.py`
...
- [ ] **Add phase decision fields to TurnRecord**
  - File: `ruche/audit/models/turn_record.py`
  - Action: Modify
  - Details: Track all major brain decisions
    ```python
    class TurnRecord(BaseModel):
        # ... existing fields ...
...
- [ ] **Compute TurnOutcome in FocalBrain**
  - File: `ruche/brains/focal/engine.py`
  - Action: Add method
  - Details: Determine resolution and categories from brain state
    ```python
    def _compute_turn_outcome(
        ...
        """Compute turn outcome based on brain decisions.
...
- [ ] **Test SESSION scope not persisted**
  - File: `tests/unit/alignment/test_interlocutor_data_persistence.py`
  - Action: Create
  - Details: Verify SESSION-scoped fields are skipped
    ...
    async def test_session_scoped_fields_not_persisted(
        ...
        await engine._persist_interlocutor_data(session, updates)
...
- [ ] **Test outcome computation**
  ...
  - Action: Create
  - Details: Verify correct resolution based on brain state
    ```python
...
- [x] **Phase 1-10 Complete**: All prior brain phases working
...
- [ ] **InterlocutorDataUpdate Model**: For Phase 3 → Phase 11 handoff
...
- [ ] **Add persistence config section**
  ...
  - Details: Control persistence behavior
    ...
    interlocutor_data_enabled = true
...
- [ ] **Add PersistenceConfig model**
  ...
  - Details: Persistence configuration model
    ...
    class PersistenceConfig(BaseModel):
        ...
        interlocutor_data_enabled: bool = Field(
            default=True,
...
1. ✅ **Parallel Persistence**: Session, InterlocutorData, and TurnRecord save concurrently
