...
> **Purpose**: This document defines the execution protocol for ALL subagents implementing the focal turn brain phases.
...
4. **`docs/focal_brain/implementation/CHECKLIST_CORRECTIONS.md`** - Naming consolidation (CRITICAL)
5. **Your assigned phase checklist** - `phase-{N}-{name}-checklist.md`
6. **Ground yourself in the focal turn brain specs** - Read the design docs in `docs/focal_brain/spec/`
...
| Wrong ❌ | Right ✅ |
|----------|----------|
| Create `NewInterlocutorDataStore` alongside existing `InterlocutorDataStoreInterface` | Rename/extend existing `InterlocutorDataStoreInterface` |
| Add `my_validator.py` when `validation.py` exists | Add your validation to existing `validation.py` |
| Create `InterlocutorDataUpdater` when an existing updater does similar | Rename/extend the existing updater |
...
The focal turn brain specification is split across multiple files for easier navigation:

### Main Index
- **`docs/focal_brain/README.md`** - Overview, diagrams, phase summary

### Detailed Specifications (in `docs/focal_brain/spec/`)

| File | Contents |
|------|----------|
| **`brain.md`** | Detailed Phase 1-11 specifications with substeps, inputs/outputs |
| **`data_models.md`** | All data model definitions (TurnContext, SituationalSnapshot, etc.) |
| **`configuration.md`** | Brain configuration patterns |
...
2. **`brain.md`** - Detailed substeps for your phase
...
| Document | Purpose |
|----------|---------|
| `docs/focal_brain/analysis/gap_analysis.md` | What's implemented vs. missing |
| `docs/design/scenario-update-methods.md` | Scenario migration details (Phase 1.7) |
| `docs/design/interlocutor-data.md` | InterlocutorDataStore context |
...
The existing `ruche/interlocutor_data/` module IS the InterlocutorDataStore. Do NOT create duplicate models.
...
2. Added config section `[brain.my_feature]` to `config/default.toml`
...
- `ruche/config/models/brain.py` (added config model)
