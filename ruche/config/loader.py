...
def get_config_dir() -> Path:
    ...
    The config directory can be overridden with RUCHE_CONFIG_DIR env var.
    Defaults to 'config/' relative to the project root.
    """
    config_dir_env = os.environ.get("RUCHE_CONFIG_DIR")
    if config_dir_env:
...
def get_environment() -> str:
    """Get the current environment from RUCHE_ENV.

    Defaults to 'development' if not set.
    """
    return os.environ.get("RUCHE_ENV", "development")
...
def load_config() -> dict[str, Any]:
    ...
    2. config/{RUCHE_ENV}.toml (optional)
    ...
    if not default_path.exists():
        raise FileNotFoundError(
            ...
            "Create config/default.toml or set RUCHE_CONFIG_DIR."
        )
