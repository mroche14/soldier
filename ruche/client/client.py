...
Provides a Python client for interacting with the Focal API.
...
Usage:
    ...
    async with FocalClient.dev(tenant_id="my-tenant") as client:
        agent = await client.create_agent(name="My Agent")
...
from ruche.api.models.chat import ChatRequest, ChatResponse
...
from ruche.api.models.health import HealthResponse


class FocalClientError(Exception):
...
class FocalClient:
    """Async client for the Focal API.

    Attributes:
        base_url: Base URL of the Focal API
    ...
    def dev(
        ...
        Generates a JWT token using the provided secret or RUCHE_JWT_SECRET env var.

        Args:
            tenant_id: Tenant ID for the token
            base_url: Base URL of the Focal API
            secret: JWT secret (defaults to RUCHE_JWT_SECRET env var)
            timeout: Request timeout in seconds

        Returns:
            Configured FocalClient with auth token
        """
        from jose import jwt

        jwt_secret = secret or os.environ.get("RUCHE_JWT_SECRET")
        if not jwt_secret:
            raise FocalClientError("RUCHE_JWT_SECRET not set for dev mode")
    ...
    async def __aenter__(self) -> "FocalClient":
        return self
    ...
    async def chat(
        ...
        *,
        ...
        tid = tenant_id or self.tenant_id
        if not tid:
            raise FocalClientError("tenant_id is required for chat")
