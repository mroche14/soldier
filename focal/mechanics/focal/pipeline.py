"""FOCAL Cognitive Pipeline - Main orchestrator for the FOCAL alignment mechanic.

This module implements the FocalCognitivePipeline class which orchestrates
all 12 phases of the FOCAL processing flow, from customer identification
through response generation and persistence.

The FOCAL pipeline coordinates:
...
Each step can be enabled/disabled via configuration and is instrumented
for observability with timing metrics and structured logging.
"""
...
from focal.alignment.context import SituationSensor, Turn
from focal.alignment.context.situation_snapshot import SituationSnapshot
from focal.alignment.customer import CustomerDataUpdater
...
from focal.alignment.filtering import RuleFilter, ScenarioFilter
from focal.alignment.filtering.models import MatchedRule, ScenarioFilterResult
...
from focal.alignment.migration.field_resolver import MissingFieldResolver
from focal.alignment.migration.models import (
    FieldResolutionResult,
    ReconciliationAction,
    ReconciliationResult,
)
from focal.alignment.models import Rule, Template, TurnContext
from focal.alignment.models.outcome import TurnOutcome
from focal.alignment.planning import ResponsePlanner
from focal.alignment.planning.models import ResponsePlan, ScenarioContributionPlan
...
from focal.alignment.retrieval.models import RetrievalResult, ScoredEpisode, ScoredScenario
from focal.alignment.stores import AgentConfigStore
from focal.alignment.templates_loader import load_templates_for_rules
...
from focal.conversation.models import Session, StepVisit
from focal.conversation.models.turn import ToolCall
from focal.conversation.store import SessionStore
from focal.customer_data.models import VariableEntry
...
from focal.memory.retrieval import MemoryRetriever
from focal.memory.retrieval.reranker import MemoryReranker
...
from focal.providers.rerank import RerankProvider
...
class FocalCognitivePipeline:
    ...
    The FocalCognitivePipeline orchestrates all 12 phases of the FOCAL
    (Focused Contextual Alignment) processing flow:

    Phase 1:  Identification & Context Loading
    Phase 2:  Situational Sensor
    Phase 3:  Interlocutor Data Update
    Phase 4:  Retrieval (rules, scenarios, intents, memory)
    Phase 5:  Reranking
    Phase 6:  LLM Filtering
    Phase 7:  Scenario Orchestration
    ...
    async def _process_turn_impl(
        ...
        if persist:
            from focal.observability.metrics import PERSISTENCE_DURATION
    ...
    async def _persist_turn_record(
        ...
        from focal.observability.metrics import PERSISTENCE_OPERATIONS
    ...
    async def _sense_situation(
        ...
        try:
            ...
            if self._customer_data_loader:
                ...
                from focal.customer_data import CustomerDataStore
