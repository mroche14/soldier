...
from focal.api.models.chat import ChatRequest, ChatResponse
...
from focal.api.models.health import HealthResponse
...
class FocalClient:
    ...
    def dev(
        ...
        Generates a JWT token using the provided secret or FOCAL_JWT_SECRET env var.

        Args:
            tenant_id: Tenant ID for the token
            base_url: Base URL of the Focal API
            secret: JWT secret (defaults to FOCAL_JWT_SECRET env var)
            timeout: Request timeout in seconds
        ...
        jwt_secret = secret or os.environ.get("FOCAL_JWT_SECRET")
        if not jwt_secret:
            raise FocalClientError("FOCAL_JWT_SECRET not set for dev mode")
