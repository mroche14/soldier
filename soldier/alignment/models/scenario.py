...
class StepTransition(BaseModel):
    ...
    condition_embedding: list[float] | None = Field(
        default=None, description="Vector for matching"
    )
...
class ScenarioStep(BaseModel):
    ...
    template_ids: list[UUID] = Field(
        default_factory=list, description="Step-scoped templates"
    )
    rule_ids: list[UUID] = Field(
        default_factory=list, description="Step-scoped rules"
    )
    ...
    collects_profile_fields: list[str] = Field(
        default_factory=list, description="Fields collected"
    )
...
class Scenario(AgentScopedModel):
    ...
    steps: list[ScenarioStep] = Field(
        default_factory=list, description="All steps in scenario"
    )
    ...
    content_hash: str | None = Field(
        default=None, description="SHA256 of serialized content"
    )
