...
class Episode(BaseModel):
    ...
    group_id: str = Field(
        ..., description="Composite key: tenant_id:session_id for isolation"
    )
    ...
    source: str = Field(
        ..., description="Origin: user, agent, system, external"
    )
    ...
    recorded_at: datetime = Field(
        default_factory=utc_now, description="When we learned it"
    )
    embedding: list[float] | None = Field(
        default=None, description="Semantic vector"
    )
    embedding_model: str | None = Field(
        default=None, description="Model that generated vector"
    )
